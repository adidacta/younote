...
const YOUNOTE_API_URL = 'http://localhost:3000/api'; // Change to production URL when deploying
...
async function handleCreateNote(data) {
  try {
    ...
    const pageId = await getOrCreatePage(result.authToken, videoId, videoTitle, videoUrl, notebookId);
    if (!pageId) {
...
async function getOrCreateNotebook(authToken) {
  try {
    ...
    if (result.notebookId) {
      ...
      const response = await fetch(`${YOUNOTE_API_URL}/notebooks/${result.notebookId}`, {
        headers: {
...
async function getOrCreatePage(authToken, videoId, videoTitle, videoUrl, notebookId) {
  try {
    // Search for existing page by video ID
    const response = await fetch(`${YOUNOTE_API_URL}/pages?youtube_video_id=${videoId}`, {
      headers: {
        'Authorization': `Bearer ${authToken}`,
      },
    });
